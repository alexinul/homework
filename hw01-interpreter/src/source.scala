object Source extends App {
  ...
  private def test2 = {
    val lambda = Lambda(List("n"), BinaryOperation(Add, Val("n"), Const(2)))
    Console.println(interpreter.apply(Apply(lambda, Map("n" -> BinaryOperation(Mul, Val("x"), Val("y")))), Map("x" -> Const(2), "y" -> Const(3))))
  }
}
